...
yaw = heading(1:index:length(filtered_points.Var1)-1);

if(moreLaps)
    yaw = [yaw(end-meters/delta_s:end);yaw(2:end)];
end

cones = readtable('Germany2019.txt'); % should be the input cones file, Germany2019 is given as example
yellow = [];
blue = [];
